 ...
 * \获取当前TU非零残差系数的总数
 *
 * \param  psCoef        存储当前TU残差系数的数组
 * \param  nStride       当前TU每行残差系数存储空间的大小（这里与nSize一致）
 * \param  nSize         当前TU的边长
 */
 ...
 * \编码CU分割信息
 *
 * \param  h             存储所有编码信息的结构体
 * \param  pCache        存储所有语法元素及相关信息的结构体
 * \param  pCabac        控制cabac编码的结构体
 * \param  pBS           控制码流书写的结构体
 * \param  uiAbsPartIdx  当前CU在LCU中的Z顺序索引
 * \param  uiDepth       当前CU深度
 */
...
/**
 * \编码预测信息（帧内：亮度和色度预测模式； 帧间：参考帧信息和运动矢量）
 *
 * \param  h             存储所有编码信息的结构体
 * \param  pCache        存储所有语法元素及相关信息的结构体
 * \param  pCabac        控制cabac编码的结构体
 ...
 *
 * \param  pCache        存储所有语法元素及相关信息的结构体
 * \param  nWidth        当前TU的宽度
 * \param  bIsIntra      当期TU是否帧内TU
 * \param  bIsLuma       当期TU中的残差系数是否为亮度残差系数
 * \param  nMode         当前TU对应PU的预测模式
 * \param  uiAbsPartIdx  当前TU所在CU在LCU中的Z顺序索引
 */
UInt getCoefScanIdx( MTC265_Cache *pCache, UInt nWidth, UInt8 bIsIntra, UInt8 bIsLuma, UInt nMode, UInt uiAbsPartIdx )
{
    UInt uiCTXIdx;
    UInt nScanIdx;
...
/**
 * \获取非零系数标志位所对应的上下文模型的索引
 *
 * \param  psCoeff       存储当前TU残差系数的数组
 * \param  posX          当前残差系数在当前TU中的横坐标
 * \param  posY          当前残差系数在当前TU中的纵坐标
 * \param  blockType     当期TU边长以2为底取对数再减1
 * \param  nSize         当前TU的边长
 * \param  bIsLuma       当期TU中的残差系数是否为亮度残差系数
 */
...
/**
 * \获取残差系数组标志位所对应的上下文模型的索引
 *
 * \param  uiSigCoeffGroupFlag   存储残差系数组标志位的数组
 * \param  nCGPosX               当前残差系数组在当前TU中的横坐标
 * \param  nCGPosY               当前残差系数组在当前TU中的纵坐标
 * \param  nScanIdx              当期TU残差系数扫描方式
 * \param  nSize                 当前TU的边长
 */
...
/**
 * \编码一个TU中扫描顺序中最后一个残差系数的坐标
 *
 * \param  pCabac        控制cabac编码的结构体
 * \param  pBS           控制码流书写的结构体
 * \param  nPosX         最后一个残差系数的横坐标
 * \param  nPosY         最后一个残差系数的纵坐标
 * \param  nSize         当前TU的边长
 * \param  bIsLuma       当期TU中的残差系数是否为亮度残差系数
 * \param  nScanIdx      当期TU残差系数扫描方式
 */
void codeLastSignificantXY( MTC265_Cabac *pCabac, MTC265_BitStream *pBS, UInt nPosX, UInt nPosY, UInt nSize, UInt8 bIsLuma, UInt nScanIdx )
{
 ...
 * \param  pCabac        控制cabac编码的结构体
 * \param  pBS           控制码流书写的结构体
 * \param  psTempCoef    存储LCU残差系数的数组
 * \param  uiAbsPartIdx  当前TU所在CU在LCU中的Z顺序索引
 * \param  nSize         当前TU的边长
 * \param  uiDepth       当前TU所在CU深度
 * \param  bIsLuma       当前TU中的残差系数是否为亮度残差系数
 * \param  nMode         当前TU对应PU的预测模式
 */
void xEncodeCoeffNxN( MTC265_t *h, MTC265_Cache *pCache, MTC265_Cabac *pCabac, MTC265_BitStream *pBS, Int16 *psTempCoef, UInt uiAbsPartIdx, UInt nSize, UInt nDepth, UInt8 bIsLuma, UInt nMode )
{
...
void xWriteCU( MTC265_t *h, UInt uiDepth, UInt uiAbsPartIdx )
{
    ...
    if( uiDepth < puhDepth[uiAbsPartIdx] || bBoundary )
	{
		...
		for ( UInt uiPartUnitIdx = 0; uiPartUnitIdx < 4; uiPartUnitIdx++, uiAbsPartIdx+=uiQNumParts )
		{
			...
			{
				xWriteCU( h, uiDepth+1, uiAbsPartIdx );
			}
		}
        return;
	}
