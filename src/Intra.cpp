...
void xEncIntraPredLuma( MTC265_t *h, UInt nMode, UInt nSize )
{
    ...
    UInt        nLog2Size   = xLog2(nSize - 1);
    UInt        bFilter     = g_aucIntraFilterType[nLog2Size-2][nMode];                 //判断参考像素是否采用滤波后的参考像素
    Pxl         *pucRefY    = pCache->pucPixRef[bFilter];                               //取参考像素
    Pxl         *pucDstY    = pCache->pucPredY  + MAX_CU_SIZE*pCache->cuY + pCache->cuX;//取预测像素位置
    if( nMode == PLANAR_IDX ) 
	{
        xPredIntraPlanar(
            pucRefY,
...
//for cu partition
void xEncGetCUPosition( MTC265_t *h,UInt uiCUWidth, UInt8  uiDepth, UInt uiCUX, UInt uiCUY, UInt* uisubCUX, UInt* uisubCUY)
{   
	MTC265_Cache *pCache = &h->cache;
	UInt8 uiMaxCUWidth   = h->ucMaxCUWidth;
	* uisubCUX    += h->uiCUX;
	* uisubCUY    += h->uiCUY;
	Int * PartIdx  = pCache->PartIdx;//PartIdx 为一个块划分成4个子块对应的序号0,1,2,3
...
void xEncIntraCompressCU(MTC265_t *h,UInt8 uiDepth, UInt32 uiCUX, UInt32 uiCUY)
{
	MTC265_Cache  *CU_Cache   = &h->cache;
	UInt32 iLCUX              = uiCUX;                      //当前CU的X坐标
	UInt32 iLCUY              = uiCUY;                      //当前CU的Y坐标
	Int * PartIdx             = CU_Cache->PartIdx;          //相对索引0-3
	Int * RelIdx              = CU_Cache-> RelativeIdx;     //Z扫描索引号
	UInt nCUSize              = (h->ucMaxCUWidth)>>uiDepth;
	bool SplitFlag            = true;
	{
		xEncCheckRDCost(h,SIZE_2Nx2N,uiDepth, uiCUX,  uiCUY); //计算SIZE_2N*2N的RDcost
		if(uiDepth == h->ucMaxCUDepth-1)                      //达到最大深度-1才存在SIZE_N*N
		{   		  
			xEncCheckRDCost(h,SIZE_NxN,uiDepth, uiCUX,  uiCUY); 	
		}
	}
	if( SplitFlag == true&&uiDepth<h->ucMaxCUDepth-1)        //划分成四个子块递归计算RD_COSR
	{ 
		...
		{   
			...
			UInt32 RelativIdx=0;
			for(Int32 num=0;num<=Int32(nextDepth);num++)        //计算子块的Z扫描顺序
			{
			...
			xEncIntraCompressCU(h,nextDepth, subCUX, subCUY);                                                //递归
		}
		//  计算当前深度的失真、比特数、总cost
		subCUX=0;
	  	subCUY=0;
		...
		xEncCheckBestMode(h,nextDepth,nCUSize );             //比较当前块划分与不划分的最优情况
		xEncCacheStoreCU(h,subCUX,subCUY,nCUSize,uiDepth);   //将最佳CU中数据更新重建图像等信息;   
	}
}

void xEncCheckRDCost(MTC265_t *h, MTC265_PartSize ePartSize, UInt8 uiDepth,UInt32 uiCUX, UInt32 uiCUY)
{
	 MTC265_Cache  *CU_Cache     = &h->cache;
	 UInt32 nCUSize = h->ucMaxCUWidth>>uiDepth;
	 Int *   RelIdx = CU_Cache->RelativeIdx;
	 Int *  PartIdx = CU_Cache->PartIdx;
	 if(ePartSize == 0)//SIZE_2N*2N
	 {
		 setPartSizeSubParts(CU_Cache,ePartSize, CU_Cache->RelativeIdx[uiDepth], uiDepth );
		 setPredModeSubParts(CU_Cache, MODE_INTRA, CU_Cache->RelativeIdx[uiDepth], uiDepth ); //设置预测模式
		 estIntraPredQT(h,nCUSize,uiDepth,ePartSize);                                         //进行帧内预测
		 setCUPameters( CU_Cache, uiDepth );                                                  //保存各种信息
		 xEncCacheStoreCU( h, uiCUX, uiCUY, nCUSize, uiDepth );                               //保存重建像素
	 }
	 else if(ePartSize == 3)//SIZE_N*N
	 {		
		 ...
		 for(int i=0;i<4;i++)
		 {
			...
			xEncGetCUPosition(h,subPUWidth, uiDepth+1,h->uiCUX,h->uiCUY, &subPUX, &subPUY);        //计算子块CU位置                                           		
			xEncIntraLoadRef(h, subPUX, subPUY, subPUWidth,uiDepth+1,RelativIdx,RelIdx[uiDepth]);  //加载参考像素
			estIntraPredQT(h,subPUWidth,uiDepth+1,ePartSize);                                      //进行帧内预测
		    setCUPameters(CU_Cache,uiDepth+1 );                                                    //保存各种信息
			xEncCacheStoreCU( h,subPUX,subPUY,subPUWidth,uiDepth+1);                               //保存重建像素
		 }
		 subPUX = 0;
	  	 subPUY = 0;
		 ...
		 xEncCheckBestMode(h,uiDepth+1,nCUSize);	                                   //比较当前CU划分与不划分的最优情况	 
		 xEncCacheStoreCU(h,subPUX,subPUY,subPUWidth<<1, uiDepth );                    //保存最优的重建像素
	 }
}

void xEncCheckBestMode(MTC265_t *h, UInt8  Depth, UInt32  uiCUWidth)//check 当前CU划分与不划分的最优
{
	if(Depth==0)
	{
		return;
	...
	if(CurrTotalCost <LDepthTotalCost || cTemppePartSize[stride]== SIZE_NONE)//如果划分成4个子块为最优则将信息保存到上一层的数组里
	{
...
void estIntraPredQT(MTC265_t *h,UInt32 nCUSize,UInt8 uiDepth ,MTC265_PartSize ePartSize)//帧内预测函数
{
	MTC265_Cache  *pCache  = &h->cache;
	...
	for( nrealMode=0; nrealMode<35; nrealMode++ ) 
	{
	    nMode = nrealMode;//CandMode[nrealMode];
		double uiSad;
		xEncIntraPredLuma( h, nMode, nCUSize );  //亮度预测
		//用SAD计算COST
	...
	xEncIntraPredLuma( h, nBestModeY, nCUSize );   //使用最佳模式预测
	xSubDct( piTmp0,
		pucPixY,
		pucPredY, MAX_CU_SIZE,
		piTmp0, piTmp1,
		nCUSize, nCUSize, nBestModeY );                                               //DCT
	uiSumY = xQuant( piCoefY, piTmp0, MAX_CU_SIZE, nQP, nCUSize, nCUSize, SLICE_I );  //量化

	uiBestSadY = 0;      //这里用量化后的系数的绝对值和计算SAD 为checkBestMode准备
	...
	if( uiSumY ) 
	{
		xDeQuant( piTmp0, piCoefY, MAX_CU_SIZE, nQP, nCUSize, nCUSize, SLICE_I );   //反量化
		
		xIDctAdd( pucRecY,
			piTmp0,
			pucPredY, MAX_CU_SIZE,
			piTmp1, piTmp0,
			nCUSize, nCUSize, nBestModeY );                                         //反DCT
	}
	...
	*pCbfY = (uiSumY != 0);
	...
	{		
		if(ePartSize == 3)//如果到4*4亮度情况下 色度其实对应的是8*8的情况即色度大小也是4*4只需要预测一次，取partidx=0的亮度块预测模式
		{
			nCUSize = nCUSize<<1;
			flagC   =  true;
		...
		for( nMode=0; nMode<NUM_CHROMA_MODE; nMode++ ) 
		{
			...
			xEncIntraPredChroma( h, realModeC, nCUSize >> 1);//色度预测
...
void xEncIntraLoadRef( MTC265_t *h, UInt32 uiX, UInt32 uiY, UInt32 nSize ,UInt8 uiDepth,UInt32 uiCurrPartUnitIdx,UInt32 uiAbsIdxInLCU)
 {
	MTC265_Cache  *pCache       = &h->cache;
    ...
    Pxl *pucRefY0               = pCache->pucPixRef[0];   //Y参考像素
    Pxl *pucRefY1               = pCache->pucPixRef[1];   //滤波后的Y参考像素
    Pxl *pucRefU                = pCache->pucPixRefC[0];  //U参考像素
    Pxl *pucRefV                = pCache->pucPixRefC[1];  //V参考像素
	UInt8 *pucTopModeY          = (pCache->pucTopModeY)+(h->uiCUX)*(pCache->NumPartition)/(h->ucMaxCUWidth); //获取上块LCU预测模式
	UInt8 *pucLeftModeY         = pCache->pucLeftModeY;                                                      //获取左块LCU预测模式
	UInt  uiNumPartInCUWidth    = pCache->NumPartitionInWidth;
    UInt  uiNumPartition        = pCache->NumPartition;
	UInt8 * TemppuhLumaIntraDir   = pCache->TemppuhLumaIntraDir+uiNumPartition*uiDepth;                      //获取当前LCU亮度预测模式
	UInt8 * TemppuhChromaIntraDir = pCache->TemppuhChromaIntraDir+uiNumPartition*uiDepth;                    //获取当前LCU色度预测模式
	Int   * PartIdx = pCache->PartIdx;
	Int   iLeftIntraDir, iAboveIntraDir;
	Int   RelativIdxUpRight,RelativIdxLeftBottom; 
	UInt8 *pucMostModeY    =  pCache->ucMostModeY;         //MPM
	...
	{
		if(uiCurrPartUnitIdx == 0) //Z扫描顺序为0只需要考虑上LCU和左LCU中的预测模式 
		{
		...
		{   //第一行Z扫描序号的子块上边预测模式从上LCU里查，左边预测模式从当前LCU里查
		...
		{   //第一列Z扫描序号的子块左边预测模式从左LCU里查，上边预测模式从当前LCU里查
	...
	if(pCache->TemppePartSize[uiDepth*(pCache->NumPartition)+uiCurrPartUnitIdx] == SIZE_2Nx2N) //if(nSize>4)
	{
		...
		else 
		{
			...
			// Filter with [1 2 1]  pucRefY0存放原始参考像素 pucRefY1存放滤波后的参考像素
	...
	{	  //4*4情况下 色度块只需预测一次 仅需要重新加载亮度的参考像素
...
void setPartSizeSubParts( MTC265_Cache *pCache, MTC265_PartSize eMode, UInt uiAbsPartIdx, UInt8 uiDepth ) //存储划分类型
{
...
void setPredModeSubParts( MTC265_Cache *pCache, MTC265_PredMode PredMode, UInt uiAbsPartIdx, UInt8 uiDepth )
{
	int ipow         = int(pow(double(4),uiDepth));
	Int NumPartition = (pCache->NumPartition);
	Int stride       = NumPartition /ipow;
...
void setCUPameters( MTC265_Cache *pCache, UInt8 uiDepth )   //保存各种参数信息
{
	Int RelativeIdx = pCache->RelativeIdx[uiDepth];
	UInt8 * TempbCbfY = pCache->TempbCbfY;                           //存储每一深度的CbfY
	UInt8 * TempbCbfU = pCache->TempbCbfU;                           //存储每一深度的CbfU
	UInt8 * TempbCbfV = pCache->TempbCbfV;                           //存储每一深度的CbfV
	...
	UInt8  bCbfY = pCache->bCbfY;                                    //Y残差系数是否全为0
    UInt8  bCbfU = pCache->bCbfU;                                    //U残差系数是否全为0
	UInt8  bCbfV = pCache->bCbfV;                                    //V残差系数是否全为0
	double  TotalCost = pCache->TotalCost;
