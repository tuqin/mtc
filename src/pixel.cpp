...
UInt32 xSad_new( Int N, Int M,Pxl *pSrc, UInt nStrideSrc, Pxl *pRef, UInt nStrideRef )//自己加的求SAD函数
{
    Int x, y;
    UInt32 uiSad = 0;
...
double CalDistortion(Pxl * pSrc, Pxl * pPred, Int32 stride, UInt32 CUSize)
{
	...
	for (UInt32 i=0; i<CUSize;i++)
	{
		...
		{
			dist += double(abs(pSrc[i*stride+j]-pPred[i*stride+j]));
		}
	}
	return dist;
}
...
double CalInterDistortionSSE(Pxl * pSrc, Pxl * pPred, Int32 strideSrc, Int32 stridePred, UInt32 CUSize, MTC265_PartSize ePartSize)
{
	...
	if (ePartSize == SIZE_2Nx2N)
	{
		for (UInt32 i=0; i<CUSize;i++)
		{
			for (UInt32 j=0; j<CUSize;j++)
			{
				dist = dist + double(abs(pSrc[i*strideSrc+j]-pPred[i*stridePred+j]) * abs(pSrc[i*strideSrc+j]-pPred[i*stridePred+j]));
			}
		}
		return dist;
	}
